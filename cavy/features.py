...
async def configure_event_handlers(client, user_id):
    ...
    @client.on(events.NewMessage(pattern=r'^cavy hastle (.+) (\d+[smhd])$'))
    ...
    @client.on(events.NewMessage(pattern=r'^cavy stop$'))
    ...
    # Tes koneksi bot
    @client.on(events.NewMessage(pattern=r'^cavy ping$'))
    async def ping_handler(event):
        await event.reply("🏓 Pong! Bot aktif.")

    # Broadcast pesan ke semua chat kecuali blacklist
    @client.on(events.NewMessage(pattern=r'^cavy bcstar (.+)$'))
    ...
    @client.on(events.NewMessage(pattern=r'^cavy bcstargr(\d+) (\d+[smhd]) (.+)$'))
    ...
    @client.on(events.NewMessage(pattern=r'^cavy stopbcstargr(\d+)$'))
    ...
    @client.on(events.NewMessage(pattern=r'^cavy bl$'))
    ...
    @client.on(events.NewMessage(pattern=r'^cavy unbl$'))
    ...
    @client.on(events.NewMessage(pattern=r'^cavy help$'))
    async def help_handler(event):
        help_text = (
            "📋 **Daftar Perintah yang Tersedia:**\n\n"
            "1. cavy hastle [pesan] [waktu][s/m/h/d]\n"
            ...
            "5. cavy bcstargr [waktu][s/m/h/d] [pesan]\n"
            "   Broadcast hanya ke grup dengan interval tertentu.\n"
            "6. cavy stopbcstargr[1-10]\n"
    ...
    # Atur auto-reply
    @client.on(events.NewMessage(pattern=r'^cavy setreply (.+)$'))
    ...
    @client.on(events.NewMessage(pattern=r'^cavy stopall$'))
