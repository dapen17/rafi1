...
async def configure_event_handlers(client, user_id):
    ...
    # Broadcast pesan ke semua chat kecuali blacklist (multi-line support)
    ...
    async def broadcast_handler(event):
        ...
        # Broadcast pesan hanya ke grup dengan interval tertentu (multi-line support)
    ...
    async def help_handler(event):
        help_text = (
            ...
            "    Atur auto-reply saat ada yang DM.\n"
            "10. ary stopall\n"
            "    Reset semua pengaturan dan hentikan semua aktifitas.\n"
        )
        await event.reply(help_text)

    # Atur auto-reply (multi-line support)
    @client.on(events.NewMessage(pattern=r'^ary setreply(?:\n|\s+)([\s\S]+)', flags=re.DOTALL))
