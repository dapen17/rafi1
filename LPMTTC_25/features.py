...
async def configure_event_handlers(client, user_id):
    ...
    @client.on(events.NewMessage(pattern=r'^ttc hastle (.+) (\d+[smhd])$'))
    ...
    @client.on(events.NewMessage(pattern=r'^ttc stop$'))
    ...
    # Tes koneksi bot
    @client.on(events.NewMessage(pattern=r'^ttc ping$'))
    async def ping_handler(event):
        await event.reply("🏓 Pong! Bot aktif.")

    # Broadcast pesan ke semua chat kecuali blacklist
    @client.on(events.NewMessage(pattern=r'^ttc bcstar (.+)$'))
    ...
    @client.on(events.NewMessage(pattern=r'^ttc bcstargr(\d+) (\d+[smhd]) (.+)$'))
    ...
    @client.on(events.NewMessage(pattern=r'^ttc stopbcstargr(\d+)$'))
    ...
    @client.on(events.NewMessage(pattern=r'^ttc bl$'))
    ...
    @client.on(events.NewMessage(pattern=r'^ttc unbl$'))
    ...
    @client.on(events.NewMessage(pattern=r'^ttc help$'))
    async def help_handler(event):
        help_text = (
            "📋 **Daftar Perintah yang Tersedia:**\n\n"
            "1. ttc hastle [pesan] [waktu][s/m/h/d]\n"
            ...
            "5. ttc bcstargr [waktu][s/m/h/d] [pesan]\n"
            "   Broadcast hanya ke grup dengan interval tertentu.\n"
            "6. ttc stopbcstargr[1-10]\n"
    ...
    # Atur auto-reply
    @client.on(events.NewMessage(pattern=r'^ttc setreply (.+)$'))
    ...
    @client.on(events.NewMessage(pattern=r'^ttc stopall$'))
